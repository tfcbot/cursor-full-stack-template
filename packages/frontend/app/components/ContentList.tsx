...
import { useGetAllContent } from '../api';

export function ContentList() {
  const { data: contentList, isLoading, isError } = useGetAllContent();
  ...
  if (isError) {
    return (
      <div className="bg-error bg-opacity-10 p-6 rounded-lg text-center">
        ...
        <p className="text-fg-secondary mt-2">Could not load content at this time. Please try again later.</p>
      </div>
    );
  }
  ...
  if (!contentList || contentList.length === 0) {
    return (
      <div className="bg-bg-secondary bg-opacity-60 p-8 rounded-lg text-center border border-border">
        <h2 className="text-xl font-medium text-fg-primary">No content yet</h2>
        <p className="text-fg-secondary mt-2">Generate your first content to see it here.</p>
        <Link 
          href="/" 
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-fg-primary">Your Content</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {contentList.map((content) => (
          ...
          >
            <h2 className="text-lg font-semibold text-fg-primary mb-2 truncate">{content.topic}</h2>
            <p className="text-fg-tertiary text-sm mb-4">
              {new Date(content.createdAt).toLocaleDateString()}
            </p>
            <p className="text-fg-secondary line-clamp-3">{content.content.substring(0, 150)}...</p>
          </Link>
        ))}
      </div>
    </div>
