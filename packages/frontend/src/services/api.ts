...
export const getResearchById = async (id: string, token?: string): Promise<RequestAgentTaskOutput | null> => {
  const timestamp = new Date().getTime();
  const absoluteUrl = await getAbsoluteUrl(`/research/${id}?_t=${timestamp}`);
  try {
    const response = await fetch(absoluteUrl, {
      method: 'GET',
      headers: await getHeaders(token),
    ...
    if (data && typeof data === 'object') {
      // Check if data is directly the task object (support both taskId and researchId)
...
export const postResearch = async (requestData: RequestAgentTaskFormInput, token: string): Promise<RequestAgentTaskOutput> => {
  ...
  try {
    ...
    const optimisticTask: RequestAgentTaskOutput = {
      ...
      researchId: clientGeneratedId, // For backward compatibility
      userId: '', // Will be filled by the server
      title: `Task: ${requestData.prompt.substring(0, 50)}...`,
