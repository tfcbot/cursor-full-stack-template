...
import { RequestAgentTaskOutput } from '@metadata/agents/agent.schema';

export function ResearchList() {
  const { data: taskList, isLoading, isError } = useGetAllResearch();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {taskList.map((task: RequestAgentTaskOutput) => {
          // Support both taskId (new) and researchId (legacy)
          const id = task.taskId || task.researchId;
          return (
            <Link 
              key={id}
              href={`/research/${id}`}
